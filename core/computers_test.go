...
func TestIsValidMECTRole(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			input:  core.MECTRoleLocalMint,
			output: true,
		},
		{
			input:  core.MECTRoleLocalBurn,
			output: true,
		},
		{
			input:  core.MECTRoleNFTCreate,
			output: true,
		},
		{
			input:  core.MECTRoleNFTAddQuantity,
			output: true,
		},
		{
			input:  core.MECTRoleNFTBurn,
			output: true,
		},
		{
			input:  "mectRoleLocalMint",
			output: false,
		},
		{
			input:  "",
	...
	for _, tt := range tests {
		require.Equal(t, tt.output, core.IsValidMECTRole(tt.input))
	}
}
